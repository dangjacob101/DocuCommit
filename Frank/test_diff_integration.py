# test_diff_integration.py - week 6 deliverable
#
# integration tests for the diff engine, driven through the real HTTP api
# (GET /api/branches/<id>/diff) rather than calling diff_engine functions
# directly. this exercises the full path: request -> route -> content
# reconstruction -> extract_plain_text -> compute_visual_diff -> json.
#
# the branch-vs-main diff is the engine's main surface in the app, so we
# build a document, branch it, commit an edited version on the branch, then
# diff the branch against Main and assert on the summary the api returns.
#
# focuses on the edge cases that tend to break diff engines:
# entirely-deleted documents, massive copy-pastes, huge files with a single
# tiny change, far-apart edits, whitespace-only noise, unicode/emoji, and
# pathologically long single lines.
#
# uses flask's test_client against a temp sqlite db so each run is clean
...
# branches and commits reference each other, so sqlite can't sort tables
# for DROP. the drop still works; silence the warning to keep output clean.
...
def test_unicode_and_emoji_content():
    """accented characters, currency symbols, and emoji must not break the diff.

    the test data deliberately contains real unicode (e-acute, euro sign,
    grinning + crying face emoji) since that is exactly what we are testing.
    """
    client = fresh_client()
    branch = doc_with_edited_branch(
        client, "Unicode",
