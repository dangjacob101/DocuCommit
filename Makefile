# DocuCommit - common developer tasks.
#
# quick start for a fresh clone:
#   make install && make check
#
# (Make is used here for the convention the course recommends: one place to
#  define install/test/check commands so everyone runs the project the same
#  way, even though Python/JS are not compiled.)

.PHONY: help install backend-deps frontend-deps test e2e check clean
...
help:
	...
	@echo "  make check     run the full health check (all tests, with summary)"
